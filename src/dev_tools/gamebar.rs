...
pub mod windowsonly {
    ...
    use windows::Graphics::{Capture::GraphicsCaptureItem, DisplayId};
    ...
    fn start_capture() {
        ...
        if let Ok(picker) = windows::Graphics::Capture::GraphicsCapturePicker::new() {
            if let Ok(itempick) =
                windows::Graphics::Capture::GraphicsCapturePicker::PickSingleItemAsync(&picker)
            {
                print!("start pick single item{:?}", itempick)
            }
        }

        if let Ok(item) = windows::Graphics::Capture::GraphicsCaptureItem::TryCreateFromDisplayId(
            DisplayId::default(),
        ) {}

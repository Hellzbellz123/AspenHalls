...
fn main() {
    ...
    vanillacoffee.add_plugin(VCLogPlugin {
        // filter: "".into(),
        filter: "".into(), //"bevy_ecs=warn,naga=error,wgpu_core=error,wgpu_hal=error,symphonia=warn".into(), // filters for anything that makies it through the default log level. quiet big loggers
